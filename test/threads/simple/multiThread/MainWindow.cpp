    ...
    ui(new Ui::MainWindow)
{
    ...
    theThreader_->setNumberOfThreads(ui->maxThreadsSB->value());
...
{
    STDLINE("Process A started",ACBlack) ;
    ...
    theThreader_->reset();
    for(int p=0; p<ui->maxThreadsSB->value(); p++)
    {
        theProcessA_ = new processA(p) ;
...
void MainWindow::advanceProgressBar()
{
  ...
  {
    STDLINE("All events have been processed",ACBlack) ;
    theTimer_->stop();
...
//==============================================================
void MainWindow::on_JenniferPB_clicked()
{
    for(int i=0; i< 1000000; i++)
        STDLINE("Ciao Sono Jennifer",ACBlack) ;
